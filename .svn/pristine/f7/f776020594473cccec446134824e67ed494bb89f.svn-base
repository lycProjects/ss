...
<script type="text/javascript">
	...
	$(document).ready(function(){//当文档加载完毕后执行
		...
		$("#checkedAll").click(function(){	//checkedAll绑定事件
			if($(this).prop("checked")==true){//$(this)可以使用jq方法特性，prop() 方法设置或返回被选元素的属性和值。判断checked是否被选中
				$("input[name='commentIds']").prop("checked",true);//选中name为commentIds的所有input，把他们属性改为true
			}else{
	...
	function commentsDelete(){
		var chk_value=[];//复选框选中的数据
		$('input[name="commentIds"]:checked').each(function(){//选中name为commentIds并且属性是被选中的所有input；each遍历
			chk_value.push($(this).val());/* 取到的value数据放到数组里 */
		});
		if(chk_value.length==0){//0长度为空
			alert("请选择要删除的数据");
			return;
		}
		var commentIds=chk_value.join(",");//join将 {1,2} 转成  "1,2"
	...
	function commentDelete(commentId){
		if(confirm("确认要删除这条评论吗？")){
			$.post("comment?action=delete",{commentIds:commentId},/* 参数 */
				function(result){//回调方法，传值
					var result=eval('('+result+')');/* json串转json对象 */
					if(result.success){
						alert("删除成功！");
						window.location.href="${pageContext.request.contextPath}/comment?action=backList";
					}else{
