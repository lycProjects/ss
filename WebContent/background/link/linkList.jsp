...
<script type="text/javascript">
	function linkDelete(linkId){
		if(confirm("确认要删除这条友情链接吗？")){
			$.post("link?action=delete",{linkId:linkId},//linkId参数属性key，value的linkId
				function(result){//回调方法 利用JQ ajax，比传统方便
					var result=eval('('+result+')');//json串转成json对象
					if(result.success){//前台success值是true
						alert("删除成功!");
						window.location.href="${pageContext.request.contextPath}/link?action=backList";//跳转
					}else{
